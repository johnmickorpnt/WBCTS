...
if ($settlements->save()) {
    ...
    $result = array("status" => 1, "message" => "Settlement created successfully!");
    echo json_encode($result, JSON_PRETTY_PRINT);
} else {
    // Failed to create the settlement
    $result = array("status" => 0, "message" => "Failed to create the settlement.");
    echo json_encode($result, JSON_PRETTY_PRINT);
}
?>
